...
class EasyIntersectionObserver {
  ...
  initObserver(el: Element | NodeListOf<Element> | string | Array<Element>) {
    ...
    if ((el as NodeListOf<Element> | string).length) {
      ...
      this.elList = Array.from(el as NodeListOf<Element>)
    } else if (getType(el).includes('Element')) {
      this.elList = [el as Element]
    }
  ...
  observe(el?: Element) {
    if (el) {
      ...
      this.elList.forEach((_el: Element) => this.io.observe(_el))
    }
  }

  unobserve(el?: number | Element) {
    if (el === undefined) {
...
export default {
  inserted(el: HTMLElement, { modifiers, arg, value }: DirectiveBinding, { context }: any) {
    ...
    switch (getType(value)) {
      ...
      case 'Array':
        ...
        value.forEach((res, index) => {
          typeof res === 'number' && (options[keyArr[index] as 'top'] = res)
        })
        break
      case 'Number':
        options.top = value
    ...
    const observers = [new EasyIntersectionObserver(stickyElContainer, ([{
      isIntersecting,
    ...
    if ((options.stickSide === 'both' || options.bottom > 0) && stickyElContainer.parentNode) {
      observers.push(new EasyIntersectionObserver(
        stickyElContainer.parentNode as HTMLElement,
        ([{ isIntersecting }]) => {
